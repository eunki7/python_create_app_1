...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def extractWordStart(self):
        ...
        base_url = "https://search.daum.net/search?w=tot&DA=YZR&t__nil_searchbox=btn&sug=&sugo=&q="
        ...
        #Fake User-Agent 초기화
        ua = UserAgent()
        #요청(다음 사이트의 경우 header 정보를 check 하므로, fake-agent 사용)
        req = requests.get(base_url+req_param,headers={'User-Agent':str(ua.chrome)})
        soup = BeautifulSoup(req.text, "html.parser")
        for i,e in enumerate(soup.select("div#netizen_lists_top > span > a"),1):
            self.listWidget_1.addItem(e.string)
    ...
    def extractWordStart2th(self):
        ...
        soup = BeautifulSoup(req.text, "html.parser")
        for i,e in enumerate(soup.select("div#netizen_lists_top > span > a"),1):
            self.listWidget_2.addItem(e.string)
